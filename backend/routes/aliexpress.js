...
router.get("/search", auth, async (req, res) => {
  try {
    ...
    const products = (data.data?.products || data.data?.result || []).map(p => ({
      id: String(p.productId || p.itemId || Math.random()),
      title: p.title || p.productTitle || "Unknown",
...
router.post("/import", auth, async (req, res) => {
  try {
    ...
    const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
    const { data, error } = await supabase.from("imported_products").insert({
      user_id: req.user.id,
